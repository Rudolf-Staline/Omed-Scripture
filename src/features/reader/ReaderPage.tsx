  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <header className="sticky top-0 z-20 -mx-4 mb-5 border-b border-border bg-bg-primary/92 px-4 py-3 shadow-[0_18px_60px_-54px_var(--color-shadow)] backdrop-blur-xl sm:-mx-6 sm:mb-5 sm:px-6 sm:py-3 md:-mx-10 md:px-10 lg:-mx-14 lg:px-14">
        ...
        {/* Sélecteur de passage : un cluster distinct des modes de lecture */}
        <div className="flex flex-col gap-2.5 md:flex-row md:items-center md:justify-between">
          <div className="grid grid-cols-2 gap-2 rounded-2xl border border-border bg-bg-card/45 p-1.5 sm:flex sm:items-center" role="group" aria-label="Choisir le passage">
            <div className="relative col-span-2 sm:col-span-1 sm:min-w-[7.5rem]">
              <select value={effectiveTranslation} onChange={(e) => navigate(`/read/${e.target.value}/${effectiveBookId}/${chapterNum}`)} className={`${selectClass} w-full border-transparent bg-transparent shadow-none`} aria-label="Traduction">
                {FEATURED_TRANSLATIONS.map((t) => <option key={t.id} value={t.id}>{t.short} · {t.name}</option>)}
              </select>
              <SelectChevron />
            </div>
            <div className="relative min-w-0 sm:min-w-[9rem]">
              <select value={effectiveBookId} onChange={(e) => navigate(`/read/${effectiveTranslation}/${e.target.value}/1`)} className={`${selectClass} w-full border-transparent bg-transparent shadow-none`} aria-label="Livre">
                {BIBLE_BOOKS.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
              <SelectChevron />
            </div>
            <div className="relative min-w-0 sm:min-w-[7rem]">
              <select value={chapterNum} onChange={(e) => navigate(`/read/${effectiveTranslation}/${effectiveBookId}/${e.target.value}`)} className={`${selectClass} w-full border-transparent bg-transparent shadow-none`} aria-label="Chapitre">
          ...
          <ActionDock label="Modes du lecteur" className="-mx-1 max-w-full overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            <button type="button" onClick={() => setShowAudio(true)} className="omed-button-ghost min-h-10 shrink-0 px-3 text-sm" aria-label="Ouvrir l'audio">
              <Headphones size={16} strokeWidth={1.5} /> <span className="hidden sm:inline">Audio</span>
            </button>
            <button type="button" onClick={() => setCompareTranslation(compareTranslation ? null : defaultCompareTranslation)} aria-label="Comparer avec une autre traduction" aria-pressed={Boolean(compareTranslation)} className={`min-h-10 shrink-0 rounded-xl border px-3 text-sm font-semibold ${compareTranslation ? 'border-accent-gold/45 bg-accent-gold/12 text-accent-gold' : 'omed-button-ghost'}`}>
              <GitCompare size={16} strokeWidth={1.5} /> <span className="hidden sm:inline">Comparer</span>
            </button>
            <button type="button" onClick={() => setStudyMode((mode) => !mode)} aria-label="Afficher le mode étude" aria-pressed={studyMode} className={`min-h-10 shrink-0 rounded-xl border px-3 text-sm font-semibold ${studyMode ? 'border-accent-gold/45 bg-accent-gold/12 text-accent-gold' : 'omed-button-ghost'}`}>
              <NotebookPen size={16} strokeWidth={1.5} /> <span className="hidden sm:inline">Étude</span>
            </button>
