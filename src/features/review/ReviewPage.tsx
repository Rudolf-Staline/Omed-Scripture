...
import { BookMarked, BookOpenText, Brain, CheckCircle2, HandHeart, ListChecks, NotebookPen } from 'lucide-react';
...
export const ReviewPage: React.FC = () => {
  ...
  return (
    <PageCanvas width="wide" className="space-y-6">
      <PageHero kicker="Aujourd’hui" title="Centre de reprise" icon={ListChecks} intro="Un seul endroit pour reprendre vos versets, études, prières et plans de lecture." actions={<Link to={currentReaderPath} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-accent-gold px-4 font-bold text-white"><BookOpenText size={18} /> Lire maintenant</Link>}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <div className="rounded-3xl border border-border bg-bg-primary/65 p-4 lg:col-span-2"><p className="text-2xl font-bold text-text-primary">{summary.headline}</p><p className="text-sm text-text-muted">priorité spirituelle</p></div>
      ...
      {summary.clear && <section className="rounded-[1.8rem] border border-dashed border-border bg-bg-card p-8 text-center"><CheckCircle2 className="mx-auto text-accent-sage" size={42} /><h2 className="mt-4 text-2xl font-bold text-text-primary">Tout est à jour</h2><p className="mx-auto mt-2 max-w-xl text-text-secondary">Aucun verset dû, aucune étude en brouillon, aucun plan actif à reprendre.</p></section>}
