...
describe('useNotesStore', () => {
  ...
  it('loads initial state from valid localStorage', async () => {
    const mockNotes = [{ id: '1', verseId: 'lsg-jean-3-16', text: 'God loves us', verseText: 'For God so loved...', dateAdded: 123, dateModified: 123 }];
  ...
  it('edits an existing note', async () => {
    const mockNotes = [{ id: 'note-1', verseId: 'lsg-jean-3-16', text: 'Old text', verseText: '...', dateAdded: 100, dateModified: 100 }];
    localStorage.setItem(OMED_STORAGE_KEYS.notes, JSON.stringify(mockNotes));
  ...
  it('deletes a note', async () => {
    const mockNotes = [{ id: 'note-1', verseId: 'lsg-jean-3-16', text: 'Text', verseText: '...', dateAdded: 100, dateModified: 100 }];
    localStorage.setItem(OMED_STORAGE_KEYS.notes, JSON.stringify(mockNotes));
  ...
  it('does not touch other localStorage keys', async () => {
    ...
    const store = await setupStore();
    store.getState().addNote({ verseId: 'lsg-jean-3-16', text: 'Test', verseText: '...' });
