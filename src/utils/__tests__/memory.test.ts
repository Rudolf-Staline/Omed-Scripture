...
import { formatDueLabel, getDueMemoryVerses, getMemoryStats, scheduleMemoryReview } from '../memory';
...
describe('memory scheduling helpers', () => {
  ...
  it('formats due dates for current, hourly and daily reviews', () => {
    const now = new Date('2026-01-02T08:00:00.000Z');
    expect(formatDueLabel('2026-01-02T07:59:00.000Z', now)).toBe('à réviser maintenant');
    expect(formatDueLabel('2026-01-02T09:00:00.000Z', now)).toBe('dans 1 h');
    expect(formatDueLabel('2026-01-04T08:00:00.000Z', now)).toBe('dans 2 j');
    expect(formatDueLabel('not-a-date', now)).toBe('date inconnue');
  });
