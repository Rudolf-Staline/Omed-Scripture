...
export type PageLayout = "default" | "dashboard" | "auth" | "reader" | "settings" | "form";
/** Patch-style state updater, like React's but accepting partials. */
export type SetState<State> = (patch: Partial<State> | ((prev: State) => Partial<State>)) => void;
...
export type PageGuardResult = boolean | {
    redirect: string;
} | {
    error: string;
};
 ...
 */
export interface PageDefinition<State extends object = Record<string, never>, Actions extends object = Record<string, never>, Data = undefined, Services = unknown> {
    id: string;
    layout?: PageLayout;
    title?: string;
    description?: string;
